...
async function main() {
  if (!(process.env.NODE_ENV === 'test')) {
    return Promise.reject(
      new Error(
        `Tests must be run in NODE_ENV=test but not in env ${process.env.NODE_ENV}`
      )
    );
  }
  ...
  await tape('Adapter Base create', async (t) => {
    try {
      ...
      t.equals(
        req.body.email,
        createdModel.email,
        'created model with field email'
      );
    } catch (err) {
      log.error(err);
    } finally {
  ...
  await tape('Adapter Base removeMultiple', async (t) => {
    try {
      ...
      let remainingUsers = await TestPersonModel.where(
        'id',
        '>=',
        0
      ).fetchAll();
  ...
  tape('Adapter Base list search and sort', async (t) => {
    try {
      ...
      req.query = `search[first_name]=${
        createdModelB.toJSON().first_name
      }&search[last_name]=${createdModelB.toJSON().last_name}`;

      await list(req, res);

      log.info('res.data', res.data);

      t.equals(1, res.data.docs.length, 'item should be found');
      t.equals(
        testPersonFixtureB.first_name,
        res.data.docs[0].first_name,
        'should be found'
      );
      ...
      t.ok(
        testPersonFixtureB.last_name.toString().trim() ===
          res.data.docs[0].last_name.toString().trim(),
  ...
  tape('Adapter Base list comparators', async (t) => {
    try {
      ...
      const updatedAt = Moment.tz(momentDate, 'Europe/Berlin').format(
        dateFormat
      );
      ...
      for (let i = 0, l = res.data.docs.length; i < l; i++) {
        log.info(`last_name ${i}:`, res.data.docs[i].last_name);
        log.info(`date      ${i}:`, res.data.docs[i].updated_at);
      }

      //log.info('momentDate                 ', momentDate);
      //log.info('res.data.docs[0].updated_at', res.data.docs[0].updated_at);

      t.true(
        momentDate < res.data.docs[0].updated_at,
        'newer/gt date should be found'
      );

      const queryLt = qs.stringify(
        {
          range: [
            {
              field: 'updated_at',
      ...
      t.notOk(res.data.docs.length, 'older/lt date should not be found');
    } catch (err) {
      log.error(err);
    } finally {
      t.end();
  ...
  await tape('Adapter Base hooks', async (t) => {
    try {
      const { list } = require('../../lib/bookshelf-adapter.js')(
        TestPersonModel,
        null,
        {
          list: {
