...
module.exports = function BaseAdapter(Model, options, hookOptions) { 
  ...
  log.warn('mk-base-adapter/lib/base-adapter is depracted');
  log.warn('instead use mk-base-adapter/lib/bookshelf-adapter');
  ...
  adapter.copy = async function copy(req, res) {

    try {
      if (adapter.hooks.copy.before) {
        adapter.hooks.copy.before(req); 
      }

      const copiedModel = await adapter.hooks.copy.exec(req);
      /*
      const newModel = new Model(req.body);
      const rawSavedModel = await newModel.save();
      const savedModel = rawSavedModel.toJSON();
      */
      if (adapter.hooks.copy.after) {
        adapter.hooks.copy.after(req, copiedModel); 
      }
      res.status(200).json(copiedModel); 
    } catch (err) {
      log.error(err); 
      res.status(500).json({error: err}); 
    }
  ...
  adapter.list = async function list(req, res) {
    try {
      ...
      const result =
        await adapter.hooks.list.exec(req, 
          withRelated);

      if (adapter.hooks.list.after) {
        adapter.hooks.list.after(req, result);
      }
