  ...
  baseAdapter.hooks.update.exec = async function updateExec(id, reqBody) {
    ...
    return new Promise ( async ( resolve, reject ) => {
      ...
      try {
        ...
        log.info('mk-base-adapter reqBody     ', reqBody);

        const foundRecord = await Model.findOne({ _id: mongoose.Types.ObjectId(id) });

        log.info('mk-base-adapter foundRecord', foundRecord);
        ...
        log.info('mk-base-adapter updatedRecord', updatedRecord);
      
        return updatedRecord;
      
      } catch (err) {
        log.error('mk-base-adapter update error', err);
        return reject(err);
      }
    });
  };
  
  baseAdapter.hooks.list.exec = async function listExec(reqQuery, withRelated) {
