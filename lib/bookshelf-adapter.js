  ...
  let proto = {
    ...
    async remove(req, res) {
      try {
        ...
        const destroyed = await Model.forge().where({'id': id}).destroy(); 
    ...
    async removeMultiple(req, res) {
      try {
        ...
        const ids = req.body;
        const  destroyed = await Model.forge().query((qb) => {
          qb.whereIn('id', ids); 
        }).destroy();
    ...
    async list(req, res) {
      try {
        ...
        const rawResult = await query.fetchPage({ page, pageSize, withRelated });
        ...
        JSONResult.pagination.nextPage = 
          JSONResult.pagination.page < JSONResult.pagination.pages ? parseInt(page) + 1 : null;
        JSONResult.pagination.prevPage = 
          JSONResult.pagination.page > 1 ? page - 1 : null;
        JSONResult.pagination.hasNextPage =  
          JSONResult.pagination.nextPage ? true :  false;
        JSONResult.pagination.hasPrevPage =  
          JSONResult.pagination.prevPage ? true :  false;

        log.debug('base-dapter pagination', JSONResult.pagination);

        JSONResult[listKey] = rawResult.toJSON({virtuals: req.query.lean !== 'true'});
