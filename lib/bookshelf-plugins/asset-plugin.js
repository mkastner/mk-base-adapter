    ...
    constructor(attributes, options) {
      ...
      this.on('saving', async (model, attrs, options) => {
        ...
        try {
          ...
          if (model[base64AttrName] || model[binaryAttrName]) {
            ...
            const fileStatTime = new Moment(saveResult.stat.mtime); 
            const saveResultMtime = Moment.tz(fileStatTime, 'Europe/Berlin').format(dateFormat);
            
            attrs[`${pluginOptions.name}_updated_at`] = saveResultMtime;  
      ...
      this.on('saved', async  (rawModel) =>  {
        try {
          ...
          if (assetFileName) {
            const converter = await ImageConverter(resolvedTargetPath);
            if (! await converter.tempFileExists(assetFileName)) {
              return Promise.resolve();
            }         
            const convertResult = 
      ...
      this.on('destroying', async (rawModel) =>  {
        
        try {

          const assetFileName = 
            rawModel.attributes[`${pluginOptions.name}_file_name`];

          if (!assetFileName) { return Promise.resolve(); }

          const model = rawModel.toJSON();

          const converter = await ImageConverter(resolvedTargetPath); 
          
          await converter.remove({
